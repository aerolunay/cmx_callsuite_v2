...
export default function CallLogTable({ refreshKey }) {
  ...
  return (
    <div className="card call-log-card">
      ...
      {!loading && !error && rows.length === 0 && <p>No calls logged yet.</p>}
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.call_log_id}>
                ...
                <td className="call-id-cell" title={row.call_id}>
                  {row.call_id ? row.call_id.slice(0, 8) : "—"}
                </td>
              </tr>
            ))}
          </tbody>
